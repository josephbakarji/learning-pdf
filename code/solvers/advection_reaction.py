...
class Simulation(object):
    ...
    def __init__(self, grid, u, C=0.8, slope_type="centered"):
        ...
        self.Src = lambda uvec: np.sqrt(uvec)
    ...
    def states(self, dt):
        ...
        for i in range(g.ilo, g.ihi+2):
            ...
            al[i] = g.a[i-1] + 0.5*g.dx*(1.0 - self.u*dt/g.dx)*slope[i-1]

            # right state on the current interface comes from zone i
            ar[i] = g.a[i] - 0.5*g.dx*(1.0 + self.u*dt/g.dx)*slope[i]
    ...
    def update(self, dt, flux):
        ...
        anew[g.ilo:g.ihi+1] = g.a[g.ilo:g.ihi+1] + \
            dt/g.dx * (flux[g.ilo:g.ihi+1] - flux[g.ilo+1:g.ihi+2]) + dt * self.Src(g.a[g.ilo:g.ihi+1])
